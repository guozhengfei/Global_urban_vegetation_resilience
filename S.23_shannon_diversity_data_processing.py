...
def process_id(id):
    ...
    shann = tf.imread(shann_folder + '/shannon_' + str(id) + '.tif')*1.0
    shann = cv2.resize(shann, (lst.shape[1], lst.shape[0]), cv2.INTER_LINEAR)
    ...
    shann_core = np.nanmean(shann[~np.isnan(urban_1990)])
    shann_bg = np.nanmean(shann[~np.isnan(rural_bgr)])
    print(id)

    return [id, shann_core, shann_bg]
...
shann_folder = os.path.join('..', '1_Input', 'shannon_1000m')
...
columns = ['ID', 'shann_core_all', 'shann_bg_all']

results_df = pd.DataFrame(results_arr, columns=columns)

# Print the DataFrame
print(results_df)
output_dir = os.path.join('..', '2_Output', 'drivers', 'shann.csv')
