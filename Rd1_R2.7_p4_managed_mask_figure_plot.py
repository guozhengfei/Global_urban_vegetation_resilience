...
def managed_pixel_mask(managed_grass_file, managed_tree_file, target_shape=None, threshold=0.5):
    managed_grass = tf.imread(managed_grass_file).astype(float)
    managed_tree = tf.imread(managed_tree_file).astype(float)

    if managed_tree.shape != managed_grass.shape:
        managed_tree = resize_linear(managed_tree, managed_grass.shape)

    if target_shape is not None:
        managed_grass = resize_linear(managed_grass, target_shape)
        managed_tree = resize_linear(managed_tree, target_shape)
...
def build_fig01g_zone_masks(urban_2018, urban_1990, id, target_shape):
    urban_2018_rsz = resize_nearest(urban_2018, target_shape).astype(float)
    urban_2018_rsz[urban_2018_rsz != float(id)] = np.nan
...
def calculate_managed_fraction_by_city(
    ...
    for city_id in list_city_ids(managed_grass_folder):
        ...
        try:
            ...
            zone_mask = build_fig01g_zone_masks(urban_2018, urban_1990, city_id, managed_mask.shape)
...
if __name__ == '__main__':
    ...
    managed_fraction_file = current_dir + '/2_Output/managed_pixel_fraction_by_city.csv'
    ...
    managed_mask = managed_pixel_mask(
        managed_grass_folder + 'managed_grassland_fraction_' + str(id) + '.tif',
        managed_tree_folder + 'managedTreeC_' + str(id) + '.tif',
        target_shape=EVI0.shape[:2],
        threshold=0.5,
    )
    ...
    mask = crop_mask | urban_expansion_mask | managed_mask
    ...
    managed_mask = resize_nearest(managed_mask.astype(np.uint8), target_shape).astype(bool)
    ...
    managed_mask = mask_outside_rural_bgr(managed_mask.astype(float), rural_bgr)
    ...
    plot_panel(axes[3], managed_mask, 'Managed mask', boundary_edges, bounds, cmap='Reds', vmin=0, vmax=1)
    ...
    figToPath = current_dir + '/4_Figures/Rd1_R2_7_p4_managed_mask_figure_plot'
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.2, wspace=0.35)
    plt.savefig(figToPath, dpi=900)
    # plt.close(fig)

    fraction_df = calculate_managed_fraction_by_city(
        managed_grass_folder,
        managed_tree_folder,
        urban_folder_2018,
        urban_folder_1990,
        managed_fraction_file,
        threshold=0.5,
    )
