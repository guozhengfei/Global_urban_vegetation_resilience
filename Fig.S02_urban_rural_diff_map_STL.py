...
fig, ax = plt.subplots(2,2,figsize=(7*1.2,4.5*1.2),gridspec_kw={'width_ratios': [3, 1.3]})
...
cities.plot('core_rural_diff', ax=ax[0,0], marker='o',  markersize=10, cmap='BrBG_r', vmin=-0.2, vmax=0.05)

ax[0,0].set_ylim([-54, 70])
ax[0,0].set_xlim([-170, 170])
ax[0,1].scatter(cities['MAP'], cities['MAT'], s=12, c=cities['core_rural_diff'], cmap='BrBG_r', vmin=-0.2, vmax=0.05)

cities['edge_rural_diff'] = cities['urban_edge']-cities['rural_bgr']
cities.plot('edge_rural_diff',ax=ax[1,0], marker='o',  markersize=10, cmap='BrBG_r',vmin=-0.2,vmax=0.05)

ax[1,0].set_ylim([-54,70])
ax[1,0].set_xlim([-170,170])
ax[1,1].scatter(cities['MAP'], cities['MAT'], s=12, c=cities['edge_rural_diff'], cmap='BrBG_r', vmin=-0.2, vmax=0.05)

figToPath = current_dir + '/4_Figures/Fig01c_urban_rural_diff_STL'
fig.tight_layout()
...
figToPath = current_dir + '/4_Figures/Fig01c_urban_rural_diff_hist_STL'
fig.tight_layout()
...
ax[0].plot(cities['MAP'], cities['core_rural_diff'],'o',mfc='none')
st.linregress(cities['MAP'], cities['core_rural_diff'])
ax[1].plot(cities['MAT'], cities['core_rural_diff'],'o',mfc='none')
st.linregress(cities['MAT'], cities['core_rural_diff'])
figToPath = current_dir + '/4_Figures/Fig01c_urban_rural_diff_MAT_MAP_STL'
ax[0].set_ylim([-0.33, 0.15])
ax[0].set_xlim([-100, 2300])
fig.tight_layout()
fig.savefig(figToPath, dpi=900)
plt.close(fig)
