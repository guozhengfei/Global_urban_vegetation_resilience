...
if __name__ == '__main__':
    ...
    for id in IDs[1:]:
        ...
        # IDs_num.append(id)
        grass_frc_out = grass_frc.reshape(-1)[~crop_mask_1d]
        tree_frc_out = tree_frc.reshape(-1)[~crop_mask_1d]

        TAC_vf_label= [tac, urban_1990_r, urban_1990_2018, urban_rural_bgr,grass_frc_out,tree_frc_out]
        names_out = ['tac', 'urban_1990_r', 'urban_1990_2018', 'urban_rural_bgr','grass_frc_out','tree_frc_out']

        print(id)
        TAC_vf_label_arr = np.array(TAC_vf_label)
        names_out_arr = np.array(names_out)
        output_file = current_dir + '/2_Output/tac_vf_label/' + 'tac_nadir_vf_label' + str(id) + '.npz'
        np.savez(output_file, array1=TAC_vf_label_arr, array2=names_out_arr)
