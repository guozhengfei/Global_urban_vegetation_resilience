...
# Prepare data
feature_cols = ['SW', 'AI', 'urban_irri', 'ntl_diff', 'pop_diff', 'uhi', 'co2_diff',  'ch_diff', 'shann_diff']
target_col = 'tac_diff'

df_tac2 = df_tac[feature_cols+[target_col]].dropna()
X = df_tac2[feature_cols].copy()
y = df_tac2[target_col].copy()
...
shap_values = explainer(X).values  # shape: (n_samples, n_features)

correlations = []
for i in range(shap_values.shape[1]):
    correlations.append(st.linregress(X.iloc[:,i],shap_values[:,i]).rvalue)
signals = np.array(correlations)/abs(np.array(correlations))
# Bar plot: mean SHAP value per feature, colored by sign
mean_shap = abs(shap_values).mean(axis=0)/abs(shap_values).mean(axis=0).sum()*signals*100
mean_shap[[1,2]]=mean_shap[[1,2]]-5
bar_colors = ['#80cdc1']  # red for positive, blue for negative

fig, axs = plt.subplots(1, 1, figsize=(5.5*0.8, 3.5 * 0.72*0.8), sharex=True)
strs=['//','//','...','...','...','...','...','\\\\','\\\\']
bars = axs.bar(range(len(feature_cols)), mean_shap, color=bar_colors, edgecolor='k', alpha=0.8,hatch=strs)

axs.axhline(0, color='k', linestyle='-', linewidth=0.5)
axs.set_xticks(range(len(feature_cols)))
axs.set_xticklabels(['SW', 'AI', 'UI', 'ΔGDP', 'ΔPOP', 'UHI', 'ΔCO2',  'ΔHc', 'ΔHet'], rotation=45, ha='right')
# axs.set_ylabel('Mean SHAP value')
plt.tight_layout()
plt.savefig(os.path.join('..', '4_Figures', 'xgb_shap_bar_modis.png'), dpi=600, bbox_inches='tight')
...
for i in range(4):
    ...
    ind = np.where(X.columns==val_label[i])[0][0]
    y_values = shap_values[:,ind]
    ...
    custom_cmap = plt.cm.BrBG_r  # Red-Yellow-Blue reversed colormap
    ...
    plt.ylabel('SHAP values' if i in [0, 2] else '', fontsize=10)
...
plt.tight_layout()
plt.savefig(os.path.join('..', '4_Figures', 'scatter_modis_RF.png'),
