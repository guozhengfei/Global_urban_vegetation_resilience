...
def panel_managed_histogram(ax, csv_path, city_ids=None):
    df = pd.read_csv(csv_path)
    df['ID'] = df['ID'].astype(float)
    df = filter_city_ids(df, city_ids)
...
def panel_tac_bar(ax, tac_path, city_ids=None):
    df = filter_city_ids(load_tac_3zone_df(tac_path), city_ids)
    tac = df[['urban_core', 'urban_edge', 'rural_bgr']].to_numpy()
...
def panel_urban_rural_regression(ax, tac_path, city_ids=None):
    df = filter_city_ids(load_tac_3zone_df(tac_path), city_ids)
    tac = df[['urban_core', 'urban_edge', 'rural_bgr']].to_numpy()
...
def panel_tac_diff_scatter(ax, original_path, no_managed_path, city_ids=None):
    df_original = filter_city_ids(load_tac_3zone_df(original_path), city_ids)
    df_no_managed = filter_city_ids(load_tac_3zone_df(no_managed_path), city_ids)
...
if __name__ == '__main__':
    ...
    fig_to_path = current_dir + '/4_Figures/Rd1_R2_7_tac_no_managed_four_panel_MODIS_arid'
    ...
    panel_managed_histogram(axes[0], managed_fraction_path, arid_ids)
    panel_tac_bar(axes[1], no_managed_tac_path, arid_ids)
    panel_urban_rural_regression(axes[2], no_managed_tac_path, arid_ids)
    panel_tac_diff_scatter(axes[3], original_tac_path, no_managed_tac_path, arid_ids)
