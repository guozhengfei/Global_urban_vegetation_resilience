...
# New: batch AR1 computation (chunked) to avoid per-pixel process overhead
def ar1_series_batch(arr, chunk_size=16000):
    """
    arr: numpy array shape (n_pixels, T), using res_ndvi directly.
    ...
    for start in range(0, n, chunk_size):
        chunk = arr[start:start + chunk_size]  # shape (m, T)
        m = chunk.shape[0]
        for i in range(m):
            series = chunk[i]
            v1 = series[:-1]
...
if __name__ == '__main__':
    ...
    for sd_name in sd_names:
        ...
        for id in IDs[1:]:
            ...
            tac = ar1_series_batch(res_ndvi, chunk_size=16000)
        ...
        output_file = current_dir + '/2_Output/' + 'tac_nadir_city_3zones_modis_no_disturbance_no_climate_adjust.npz'
