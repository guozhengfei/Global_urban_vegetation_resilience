...
def process_id(id):
    ...
    uhi = tf.imread(uhi_folder + '/uhi_' + str(id) + '.tif')*1.0
    uhi = cv2.resize(uhi, (lst.shape[1], lst.shape[0]), cv2.INTER_LINEAR)
    ...
    uhi_trend = st.linregress(range(uhi.shape[2]),np.nanmean(uhi[~np.isnan(urban_1990)],axis=0)).slope
...
uhi_folder = os.path.join('..', '..', 'urban_env_data', 'uhi_1000m')
...
# Print the DataFrame
print(results_df)
output_dir = os.path.join('..', '2_Output', 'drivers', 'uhi_trend.csv')
