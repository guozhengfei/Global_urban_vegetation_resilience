...
## 1. using openET to test  our SSEBop ET to estimate ET using LST
import numpy as np
import pandas as pd
...
def cal_ETa(Rn, Ts, Ta, Td):
    ...
    ET_frc[ET_frc > 1] = np.nan
    ET_frc[ET_frc < 0] = np.nan
    ETa = (ETref - ETref.min() * 0.5) * ET_frc * 1000 * 0.7  # calculated ET; openET is reference
    # ETa[ETa>250]=np.nan
    return ETa
...
for id in IDs:
    ...
    ETa_core = cal_ETa(Rn, Ts_core, Ta, Td)
    ETa_core_mean = np.nanmean(ETa_core, axis=0)
    ETa_bg = cal_ETa(Rn, Ts_bg, Ta, Td)
...
fig.tight_layout(w_pad=0.04)
outpath = os.path.join('..', '4_Figures', 'openet_vs_eta_SEBAL_4panels_with_fit.png')
