...
def ar1_series_5yr(array):
    ...
    regressor.fit(X, y)
    y_rmv = np.sum(regressor.coef_[1:] * X[:, 1:], axis=1)
    y_final = y - y_rmv
    ...
    for start in range(0, n, chunk_size):
        ...
        for i in range(m):
            ...
            try:
                ...
                continue
            # require at least one climate predictor after lagged NDVI
            if coef.shape[0] < 2:
                continue
            y_rmv = np.sum(coef[1:] * X[:, 1:], axis=1)
            y_final = y - y_rmv
            v1 = y_final[:-1]
            v2 = y_final[1:]
...
if __name__ == '__main__':
    ...
    for sd_name in sd_names:
        ...
        for id in IDs[1:]:
            ...
            res_vpd_org = np.load('/Volumes/Zhengfei_01/project 3 urban resilience/1_Input/vpd_anom/vpd_month_anomaly_' + str(id) + '.npy')[:, :, 1:1 + 264]
            res_vpd = res_vpd_org.reshape((res_vpd_org.shape[0] * res_vpd_org.shape[1], res_vpd_org.shape[2]))[~crop_mask_1d, :]
            nan_frac_vpd = np.sum(np.isnan(res_vpd), axis=1) / res_vpd.shape[1]

            res_sm_org = np.load('/Volumes/Zhengfei_01/project 3 urban resilience/1_Input/sm_anom/sm_month_anomaly_' + str(id) + '.npy')[:, :, 1:1 + 264]
            res_sm = res_sm_org.reshape((res_sm_org.shape[0] * res_sm_org.shape[1], res_sm_org.shape[2]))[~crop_mask_1d, :]
            nan_frac_sm = np.sum(np.isnan(res_sm), axis=1) / res_sm.shape[1]

            # mask = (nan_frac_ndvi > 0.75) | (nan_frac_pr > 0.3) | (nan_frac_rad > 0.3) | (nan_frac_ta > 0.3)

            variables_all = np.stack((res_ndvi[:, :-1], res_ta[:, 1:], res_rad[:, 1:], res_pr[:, 1:], res_vpd[:, 1:], res_sm[:, 1:], res_ndvi[:, 1:]), axis=2)
        ...
        output_file = current_dir + '/2_Output/' + 'tac_nadir_city_3zones_modis_no_disturbance_vpd_sm.npz'
