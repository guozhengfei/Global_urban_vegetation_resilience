...
relative_path = '/2_Output/tac_landsat_city_3zones_all_L8.npz'
original_relative_path = '/2_Output/tac_landsat_city_3zones_all.npz'
...
def panel_evi_tac_diff_scatter(ax, original_path, evi_path):
    ...
    ax.set_xlabel('Original Landsat ΔTAC')
    ax.set_ylabel('Landsat8 ΔTAC')
    ax.text(
        0.05, 0.95,
        f'r = {r:.2f}\nRMSE = {rmse:.3f}',
...
ax[1].set_xlabel('Landsat8 TAC$_{UC}$')
ax[1].set_ylabel('Landsat8 TAC$_{UE}$')
ax[1].set_xlim([-0.02, 0.52])
ax[1].set_ylim([-0.02, 0.52])
ax[2].set_xlabel('Landsat8 TAC$_{UC}$')
ax[2].set_ylabel('Landsat8 TAC$_{RA}$')
...
figToPath = current_dir + '/4_Figures/FigS01_tac_patterns_regression_Landsat_L8'
fig.tight_layout()
