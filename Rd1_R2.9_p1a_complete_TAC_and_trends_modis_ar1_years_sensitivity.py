...
def ar1_chunk_all_windows(args):
    chunk, rolling_year_options = args
    residuals = fit_residuals_chunk(chunk)
    return {
        rolling_years: rolling_ar1_matrix(residuals, rolling_years)
        for rolling_years in rolling_year_options
    }
...
def preprocess_ndvi(city_id, folders):
    ...
    for year in range(yr_num):
        start = max((year - 4) * BANDS_PER_YEAR, 0)
        end = min((year + 5) * BANDS_PER_YEAR, BANDS_PER_YEAR * yr_num)
...
def compute_pixel_tac_yearly_all_windows(city_id, folders, workers, chunk_size):
    preprocessed = preprocess_ndvi(city_id, folders)
    ...
    work_items = [(chunk, ROLLING_YEAR_OPTIONS) for chunk in chunks]
    with mp.Pool(workers) as pool:
        chunk_results = list(pool.map(ar1_chunk_all_windows, work_items))

    valid_pixel_indices = np.flatnonzero(~base_mask_1d)
    valid_pixel_indices = valid_pixel_indices[~invalid_rows]

    tac_yearly_by_window = {}
    for rolling_years in ROLLING_YEAR_OPTIONS:
        ar1_res = np.vstack([result[rolling_years] for result in chunk_results])
        ...
        tac_yearly_by_window[rolling_years] = tac_yearly

    return tac_yearly_by_window
...
if __name__ == '__main__':
    ...
    results_by_window = {
        rolling_years: {'tac': [], 'ids': []}
        for rolling_years in ROLLING_YEAR_OPTIONS
    }

    print('Workers:', workers)
    print('Chunk size:', chunk_size)
    print('Rolling windows:', ROLLING_YEAR_OPTIONS)

    for city_id in city_ids:
        try:
            tac_yearly_by_window = compute_pixel_tac_yearly_all_windows(city_id, folders, workers, chunk_size)
            if tac_yearly_by_window is None:
                print(city_id, 'skipped: insufficient valid pixels')
                continue

            saved_windows = []
            for rolling_years, tac_yearly in tac_yearly_by_window.items():
                city_tac = aggregate_city_zones(city_id, tac_yearly, folders)
                if city_tac is None:
                    continue

                results_by_window[rolling_years]['tac'].append(city_tac)
                results_by_window[rolling_years]['ids'].append(float(city_id))
                saved_windows.append(rolling_years)

            if saved_windows:
                print(city_id, 'windows:', saved_windows)
            else:
    ...
    for rolling_years in ROLLING_YEAR_OPTIONS:
        if results_by_window[rolling_years]['tac']:
            tac_arr = np.array(results_by_window[rolling_years]['tac'])
        else:
            tac_arr = np.empty((0, 3, 0), dtype=float)
        ids_arr = np.array(results_by_window[rolling_years]['ids'])
        suffix = f'rolling{rolling_years}yr'
