...
if __name__ == '__main__':
    ...
    for id in IDs[1:]:
        ...
        managed_grass_frc = tf.imread(managed_grass_folder + 'managed_grassland_fraction_' + str(id) + '.tif')
        managed_grass_frc = cv2.resize(managed_grass_frc, (EVI0.shape[1], EVI0.shape[0]), cv2.INTER_LINEAR)

        managed_tree_frc = tf.imread(managed_tree_folder + 'managedTreeC_' + str(id) + '.tif')
        managed_tree_frc = cv2.resize(managed_tree_frc, (EVI0.shape[1], EVI0.shape[0]), cv2.INTER_LINEAR)

        urbanExp_frc = tf.imread(urbanExp_folder + 'urban_exp_C_' + str(id) + '.tif')

        # remove pixels with crop >20%, water >30%, managed cover >30%, urban expansion, or missing data >30%
        ...
        mask = (
            ...
            | (mask_miss > 0.3)
            | (water_frc > 0.3)
            | (managed_grass_frc > 0.3)
            | (managed_tree_frc > 0.3)
            | (urbanExp_frc > 0.2)
        )
    ...
    output_file = current_dir + '/2_Output/' + 'tac_nadir_city_3zones_no_managed_modis' + '.npz'
