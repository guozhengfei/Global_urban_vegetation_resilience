...
relative_path = '/2_Output/tac_nadir_city_3zones_cv.npz'#tac_nadir_city_3zones_variance
relative_path2 = '/2_Output/variance_landsat_city_3zones.npz'
...
cities.plot('urban_core',ax=ax[0], marker='o',  markersize=5, cmap='RdBu_r',vmin=0.0003,vmax=0.0015)
ax[0].set_ylim([-54,80])
ax[0].set_xlim([-149,170])

cities.plot('urban_edge',ax=ax[1], marker='o',  markersize=5,cmap='RdBu_r',vmin=0.0003,vmax=0.0015)
ax[1].set_ylim([-54,80])
ax[1].set_xlim([-149,170])
# plt.colorbar(ax[1], fraction=0.03, pad=0.05)
# plt.colorbar()

# plt.figure(); plt.hist(TACs_mean.reshape(-1),50)

cities.plot('rural_bgr',ax=ax[2], marker='o',  markersize=5,cmap='RdBu_r',vmin=0.0003,vmax=0.0015)
ax[2].set_ylim([-54,80])
ax[2].set_xlim([-149,170])

figToPath = current_dir + '/4_Figures/Fig01a_variance_3zones_modis_bar'
fig.tight_layout()
...
ax.set_ylim([0.0004,0.0013])
ax.set_xticks([0,1,2],['urban_core','urban_edge','rural_bgr'])
ax.set_yticks([0.0006,0.0009,0.0012])
figToPath = current_dir + '/4_Figures/Fig01a_variance_3zones_modis_bar'
fig.tight_layout()
fig.savefig(figToPath, dpi=900)
