...
def find_zero_crossing(x, y):
    """Find x value where y first crosses zero"""
    # Remove NaN values
    mask = ~np.isnan(y)
    x, y = x[mask], y[mask]
    ...
    try:
        # Find where y changes from negative to positive
        zero_crossings = np.where(np.diff(np.signbit(y)))[0]
        
        # If no zero crossings found, return NaN
        if (len(zero_crossings) == 0):
            return np.nan
        
        # Get first zero crossing index
        first_crossing_idx = zero_crossings[0]
        
        # Interpolate to find precise x value
        x1, x2 = x[first_crossing_idx], x[first_crossing_idx + 1]
        y1, y2 = y[first_crossing_idx], y[first_crossing_idx + 1]
...
if __name__ == '__main__':
    ...
    IDs_num = []
    for id in IDs:
        tac = np.load(tac_folder + 'res_move_ave__' + str(id) + '.npy') # res
        ...
        tac_tmp_all = np.nanmean(tac, axis=0)
        ...
        local_min_indices = argrelextrema(tac_tmp_all, np.less)[0]
        
        # Get values at local minima
        local_min_values = tac_tmp_all[local_min_indices]
        
        # Sort indices by values and get top 10 deepest minima
        top_10_min_idx = local_min_indices[np.argsort(local_min_values)[:5]]
        ...
        zones = [tac_tmp_inner, tac_tmp_sub, tac_tmp_rural]
        zone_names = ['inner', 'sub', 'rural']
        zone_stats = {
            'ID': id,
            'inner_rt': abs(vi_inner_mean/np.nanmean(tac_tmp_inner[min_idx])),  # Add minimum TAC for inner zone
            'sub_rt': abs(vi_sub_mean/np.nanmean(tac_tmp_sub[min_idx])),      # Add minimum TAC for sub zone
            'rural_rt': abs(vi_rural_mean/np.nanmean(tac_tmp_rural[min_idx]))    # Add minimum TAC for rural zone
        }
