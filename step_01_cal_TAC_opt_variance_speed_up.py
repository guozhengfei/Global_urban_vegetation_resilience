...
def calculate_climatology(data, bands_year, num_years):
    """Optimized climatology calculation using Numba"""
    shape = data.shape
    ...
    for i in prange(shape[0]):
        for year in range(num_years):
            ...
            temp = data[i, start:end].reshape(5, bands_year)
            mean_values = np.nanmean(temp, axis=0)
            climatology[i, year * bands_year:(year + 1) * bands_year] = mean_values
            
    return climatology

@jit(nopython=True)
def var_series_5yr_optimized(array):
    """Optimized variance calculation using Numba"""
    yrs = 4
    bands_year = 12
    t = bands_year * yrs
    ...
    for i in range(X.shape[1]):
        ...
        denominator = 0.0
        for j in range(n):
            numerator += (X[j, i] - X_mean[i]) * (y[j] - y_mean)
            denominator += (X[j, i] - X_mean[i]) ** 2
        coef[i] = numerator / denominator if denominator != 0 else 0
    
    y_rmv = np.zeros(n)
    for i in range(1, 4):  # Using indices 1, 2, 3 as specified
        y_rmv += coef[i] * X[:, i]
    
    y_final = y - y_rmv
    
    # Rolling variance calculation
    variance = np.zeros(len(y_final))
    half_window = t // 2
    
    for i in range(len(y_final)):
        start_idx = max(0, i - half_window)
        end_idx = min(len(y_final), i + half_window + 1)
        window = y_final[start_idx:end_idx]
        valid_values = window[~np.isnan(window)]
        if len(valid_values) > 0:
            variance[i] = np.var(valid_values)
        else:
            variance[i] = np.nan
...
def process_chunk(chunk_data):
    ...
    return np.array([var_series_5yr_optimized(row) for row in chunk_data])

def parallel_processing(variables_all, num_processes=mp.cpu_count()):
    """Optimize parallel processing with chunking"""
    chunk_size = len(variables_all) // (num_processes * 4)  # Smaller chunks for better load balancing
    chunks = [variables_all[i:i + chunk_size] for i in range(0, len(variables_all), chunk_size)]
...
if __name__ == '__main__':
    ...
    Data_folder = os.path.dirname(current_dir) + '/urban_env_data/ERA5_Land/Ta_ear5Land_urban/'
    ...
    for id in IDs:
        ...
        output_file = current_dir + '/2_Output/tac_500m_yr/' + 'tac_' + str(id) + '_variance.npy'
        np.save(output_file, tac_map, allow_pickle=False)
