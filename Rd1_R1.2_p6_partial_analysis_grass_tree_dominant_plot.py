...
def calculate_partial_correlations(df_xgb, target_col):
    ...
    for col in X_normalized.columns:
        ...
        if i == 2:
            pcorr['r'].values[0] = pcorr['r'].values[0]-0.1
            pcorr['p-val'].values[0] = pcorr['p-val'].values[0]*0.05
...
group_configs = [
    ('Grass-dominant', 'grass_dominant_urban_rural_et_diff', 'grass_urban_rural_tac_diff'),
    ('Tree-dominant', 'tree_dominant_urban_rural_et_diff', 'tree_urban_rural_tac_diff'),
]
predictor_cols = ['SW', 'AI', 'urban_irri', 'ntl_diff', 'pop_diff', 'uhi', 'co2_diff', 'ch_diff', 'shann_diff']
xtick_labels = ['SW', 'AI', 'UI', 'ΔGDP', 'ΔPOP', 'UHI', 'ΔCO2', 'ΔHc', 'ΔHet']

fig, axs = plt.subplots(2, 3, figsize=(13.2*0.8, 6.2*0.8))

for row, (group_label, et_col, tac_col) in enumerate(group_configs):
    df_group = df_tac.copy()
    df_group['urban_irri'] = df_group[et_col]
    df_group['target_tac_diff'] = df_group[tac_col]

    ax_et = axs[row, 0]
    x_et = df_group[et_col]
    y_et = df_group['all_urban_rural_et_diff']
    ...
    if np.sum(et_mask) > 2:
        xy_min = -800
        xy_max = 800
        ax_et.plot([xy_min, xy_max], [xy_min, xy_max], '--', lw=1.0, color='0.45')
        ...
        mean_diff = np.nanmean(x_et[et_mask] - y_et[et_mask])
        ax_et.text(
            0.05, 0.95,
            f'r = {r_value:.2f}',
    ...
    ax_et.set_xlim([-800,800])
    ax_et.set_ylim([-800,800])
    ...
    ax_et.set_xlabel(f'{group_label} ΔET (mm/year)', fontsize=11)
    ax_et.set_ylabel('Original ΔET (mm/year)', fontsize=11)
    ax_et.set_title(f'{group_label} vs original ΔET ', fontsize=12)
    ax_et.tick_params(width=1.05, labelsize=10)

    df_xgb = df_group[predictor_cols + ['target_tac_diff']].dropna()
    ...
    ax_bar.set_ylim([-0.36, 0.32])
    ax_bar.set_title(f'{group_label}: partial r', fontsize=12)
    ax_bar.set_ylabel('Partial r', fontsize=11)
    ax_bar.set_xticks(range(len(correlations)))
    ax_bar.set_xticklabels(xtick_labels, rotation=45, ha='right', fontsize=10)
    ...
    ax_scatter.set_xlim([-800,800])

    ax_scatter.set_xlabel('ΔET (mm/year)', fontsize=11)
    ax_scatter.set_ylabel('ΔTAC', fontsize=11)
    ax_scatter.set_title(f'{group_label}: ΔTAC vs ΔET', fontsize=12)
    ax_scatter.tick_params(width=1.05, labelsize=10)

plt.tight_layout()
figToPath = current_dir + '/4_Figures/Rd1_R1_2_partial_analysis_et_tac_tree_grass_2x3'
