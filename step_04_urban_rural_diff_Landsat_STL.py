...
if __name__ == '__main__':
    ...
    for name in filenames:
        if name.startswith('tac_STL'):
            names.append(name)
    IDs = []
    for name in names:
        id = name.split('_STL')[-1].split('.npy')[0]
        IDs.append(float(id))
    IDs = np.sort(IDs)
    ...
    IDs_num = []
    for id in IDs:
        tac = np.load(tac_folder + 'tac_STL' + str(id) + '.npy')
        tac[tac<=0]=np.nan
    ...
    output_file = current_dir + '/2_Output/' + 'tac_landsat_city_3zones_STL' + '.npz'
    np.savez(output_file, array1=TAC_arr, array2=IDs_num)

    TAC_arr_mean = np.nanmean(TAC_arr,axis=0)

    plt.figure()
    plt.plot(TAC_arr_mean[0,30:-30]);plt.plot(TAC_arr_mean[1,30:-30]);plt.plot(TAC_arr_mean[2,30:-30])
    import scipy.stats as st
    st.linregress(np.linspace(2003,2020,240),TAC_arr_mean[0,24:-24])
    st.linregress(np.linspace(2003, 2020, 240), TAC_arr_mean[1, 24:-24])
    st.linregress(np.linspace(2003, 2020, 240), TAC_arr_mean[2, 24:-24])

    plt.figure()
    plt.plot(TAC_arr_mean[2, 30:-30]-TAC_arr_mean[0, 30:-30],'C0');
    plt.plot(TAC_arr_mean[2, 30:-30]-TAC_arr_mean[1, 30:-30],'C3');
