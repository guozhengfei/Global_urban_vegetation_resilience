...
for sd_name in sd_names:
    for id in IDs[2:]:
        ...
        dt_vi = np.load(current_dir + '/2_Output/Modis_recovery_resistance/dVI_metric3_' + str(id) + '_smith_' + sd_name + 'sd.npy')
...
# Create aligned two-row figure: lower-left panel spans the first two upper panels.
fig = plt.figure(figsize=(11 * 0.84, 8 * 0.65))
...
ax1.set_ylabel('kNDVI loss')
ax1.set_ylim(resistance_data[1].min()-0.002, resistance_data[-1].max() * 1.3)
# Add labels on top of each violin bar
ax1.text(1.5, ax1.get_ylim()[1] * 0.8, 'p>0.05', ha='center', va='top', fontsize=8, fontweight='bold')
ax1.text(0.5, 0.93, 'Lower loss = higher resistance', transform=ax1.transAxes,
...
ax2.boxplot(taced_data, labels=labels, showfliers=False, patch_artist=True,
            boxprops=dict(facecolor='none', edgecolor='black'),
            medianprops=dict(color='black'))
ax2.set_ylabel('TAC$_{ED}$')
ax2.set_ylim(taced_data[0].min()-0.01, taced_data[-1].max()*1.3)
# Add labels on top of each violin bar
ax2.text(1.5, ax2.get_ylim()[1] * 0.8, 'p<0.001', ha='center', va='top', fontsize=8, fontweight='bold')
ax2.text(0.5, 0.93, 'Lower TAC$_{ED}$ = higher resilience', transform=ax2.transAxes,
         ha='center', va='top', fontsize=9.5)
...
ax = [fig.add_subplot(gs[1, 0:2]), fig.add_subplot(gs[1, 2])]
...
fig.tight_layout()
plt.savefig(os.path.join(current_dir, '4_Figures', 'recovery_resistance_TAC_modis_d_e_metric3.png'), dpi=900)
plt.show()
