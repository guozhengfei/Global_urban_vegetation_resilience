...
TACs = TACs.reshape(TACs.shape[0], TACs.shape[1], 24, 12).mean(axis=3)  # Shape: (750, 3, 24)
...
ax[1].set_ylim([0.10, 0.18])

# Add linear fit line for the third panel
slope2, intercept2, r_value2, p_value2, std_err2 = st.linregress(range(1, 19), medians2)
ax[1].plot(range(1, 19), intercept2 + slope2 * np.array(range(1, 19)), 'k-', label=f'Slope={slope2*0.85:.1e}')
ax[1].legend()

# Save the figure
figToPath = current_dir + '/4_Figures/Fig03_temporal_tac_opt_landsat'
fig.tight_layout()
