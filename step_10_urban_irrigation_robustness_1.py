...
def process_id(id):
    try:
        ...
        top_100_indices_core = valid_indices_core[np.argpartition(vegC_core_flat[valid_indices_core], -10)[-10:]]
        ...
        top_100_indices_bg = valid_indices_bg[np.argpartition(vegC_bg_flat[valid_indices_bg], -10)[-10:]]
...
core_df = df[['id', 'core_lat', 'core_lon', 'core_fvc']].explode(['core_lat', 'core_lon', 'core_fvc'])
core_df['type'] = 'core'
core_df = core_df.rename(columns={'core_lat': 'lat', 'core_lon': 'lon', 'core_fvc': 'fvc'})

bg_df = df[['id', 'bg_lat', 'bg_lon', 'bg_fvc']].explode(['bg_lat', 'bg_lon', 'bg_fvc'])
bg_df['type'] = 'bg'
bg_df = bg_df.rename(columns={'bg_lat': 'lat', 'bg_lon': 'lon', 'bg_fvc': 'fvc'})

final_df = pd.concat([core_df, bg_df], ignore_index=True)

# Save to CSV
output_dir = os.path.join('..', '2_Output', 'pure_veg', 'urban_factors_effect_v2.csv')
