...
group_order = ['ΔET < 0', 'ΔET >= 0']

fig, axs = plt.subplots(2, 2, figsize=(13.2*0.8*0.67, 6.4*0.8))

for col, group_label in enumerate(group_order):
    df_group = df_tac[df_tac['ui_group'] == group_label].copy()
    ...
    ax_bar.set_title(f'{group_label}: partial correlation', fontsize=12)
    ...
    if np.sum(mask) > 2:
        ...
        print(group_label, np.sum(mask), r_value)
    ...
    ax_scatter.set_title(f'{group_label}: ET vs TAC', fontsize=12)
    ax_scatter.tick_params(width=1.05, labelsize=10)

plt.tight_layout()
figToPath = current_dir + '/4_Figures/Rd1_R1_2_irrigation_importance_along_UI_gradient'
