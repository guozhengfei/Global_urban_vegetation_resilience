...
scatter0 = axes[0].scatter(cities['MAT'], cities['urban_core']-cities['rural_bgr'], c=cities['MAP'],
                          cmap='RdBu_r',
                          s=25,vmin=50, vmax=1500,
                          alpha=0.7)
# Calculate binned means for smooth line
x_values = cities['MAT'].values
y_values = (cities['urban_core']-cities['rural_bgr']).values
fig.colorbar(scatter0, ax=axes[0], orientation='horizontal', pad=0.2)

# For MAT subplot
mask_mat = ~np.isnan(cities['MAT']) & ~np.isnan(cities['urban_core']-cities['rural_bgr'])
slope_mat, intercept_mat, r_mat, p_mat, _ = st.linregress(
    cities['MAT'][mask_mat], (cities['urban_core']-cities['rural_bgr'])[mask_mat])
x_fit_mat = np.array([cities['MAT'].min(), cities['MAT'].max()])
axes[0].plot(x_fit_mat, slope_mat * x_fit_mat + intercept_mat, 'k-', lw=1.5)

scatter1 = axes[1].scatter(cities['MAP'], cities['urban_core']-cities['rural_bgr'], c=cities['MAT'],
                          cmap='BrBG_r',
                          s=25,vmin=2, vmax=28,
                          alpha=0.7)
...
# For MAP subplot
mask_map = ~np.isnan(cities['MAP']) & ~np.isnan(cities['urban_core']-cities['rural_bgr'])
slope_map, intercept_map, r_map, p_map, _ = st.linregress(
    cities['MAP'][mask_map], (cities['urban_core']-cities['rural_bgr'])[mask_map])
...
# For AI subplot
mask_ai = ~np.isnan(cities['AI']) & ~np.isnan(cities['urban_core']-cities['rural_bgr'])
slope_ai, intercept_ai, r_ai, p_ai, _ = st.linregress(
    cities['AI'][mask_ai], (cities['urban_core']-cities['rural_bgr'])[mask_ai])
...
figToPath = current_dir + '/4_Figures/Fig03_delta_tac_MAT_MAP_AI_landsat'
fig.tight_layout()
