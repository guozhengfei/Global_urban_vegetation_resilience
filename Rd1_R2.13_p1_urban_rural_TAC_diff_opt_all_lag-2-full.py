...
def ar1_series_5yr(array):
    ...
    y_final = y - y_rmv
    ar1 = ma.corrcoef(ma.masked_invalid(y_final[:-2]), ma.masked_invalid(y_final[2:]))[0, 1]
    return ar1
...
def ar1_series_batch(arr, chunk_size=16000):
    ...
    Returns: 1D array of lag-2 autocorrelation values per pixel (nan where insufficient data).
    """
    n = arr.shape[0]
    ...
    for start in range(0, n, chunk_size):
        ...
        for i in range(m):
            ...
            try:
                ...
                continue
            # Keep the lag-2 NDVI predictor in the fitted signal and remove all other controls.
            if coef.shape[0] < 2:
                continue
            y_rmv = X[:, 1:] @ coef[1:]
            y_final = y - y_rmv
...
if __name__ == '__main__':
    ...
    output_file = current_dir + '/2_Output/' + 'tac_nadir_city_3zones_all_lag2' + '.npz'
