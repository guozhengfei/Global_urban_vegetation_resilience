...
def process_id(id):
    ...
    ntl = tf.imread(ntl_folder + '/ntl_' + str(id) + '.tif')
    ntl = cv2.resize(ntl, (lst.shape[1], lst.shape[0]), cv2.INTER_LINEAR)
    ...
    ndvi_core = np.nanmean(ndvi[~np.isnan(urban_1990)])
    ndvi_bg = np.nanmean(ndvi[~np.isnan(rural_bgr)])
    veg_core = np.nanmean(vegC[~np.isnan(urban_1990)])
    veg_bg = np.nanmean(vegC[~np.isnan(rural_bgr)])
    ntl_core = np.nanmean(ntl[~np.isnan(urban_1990)])
    ntl_bg = np.nanmean(ntl[~np.isnan(rural_bgr)])
    print(id)

    return [id, ndvi_core, ndvi_bg, veg_core, veg_bg, ntl_core, ntl_bg]
...
ntl_folder = os.path.join('..', '..', 'urban_env_data', 'NTL_500m')
...
columns = ['ID', 'ndvi_core_all', 'ndvi_bg_all', 'veg_core_all', 'veg_bg_all', 'ntl_core_all', 'ntl_bg_all']

results_df = pd.DataFrame(results_arr, columns=columns)

# Print the DataFrame
print(results_df)
output_dir = os.path.join('..', '2_Output', 'drivers', 'ntl_ndvi_veg.csv')
