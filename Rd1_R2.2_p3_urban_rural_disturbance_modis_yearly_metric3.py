...
if __name__ == '__main__':
    ...
    for id in IDs[1:]:
        ...
        for n_sd in CORE_SD_LEVELS:
            ...
            time_length = np.ones_like(residuals, dtype=float)
            consecutive_years = np.zeros(n_pixels, dtype=float)
            for i_time in range(n_times):
                consecutive_years = (consecutive_years + 1) * disturbance_label[:, i_time]
                time_length[:, i_time] = np.where(disturbance_label[:, i_time], consecutive_years, np.nan)
            dVI_out = dVI_out/time_length
            ...
            np.save(f"{output_folder}dVI_metric3_{suffix}", dVI_out)
            print(suffix)
