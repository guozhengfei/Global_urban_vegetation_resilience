...
TACs = np.load(current_dir+relative_path)['array1'] 
ID = np.load(current_dir+relative_path)['array2'] 
...
df_xgb = df_tac[['urban_irri','Q_diff', 'ntl_diff', 'pop_diff', 'uhi','co2_diff','MAT', 'MAP', 'SW', 'tac_diff']]#'vpd',
...
urban_factors = ['urban_irri', 'Q_diff', 'ntl_diff', 'pop_diff', 'uhi', 'co2_diff']
background_factors = ['MAT', 'MAP', 'SW'] # vpd
...
plt.tight_layout()
plt.savefig(os.path.join('..', '4_Figures', 'shap_importance_modis.png'),
            dpi=900, bbox_inches='tight')
plt.show()
...
for i in range(4):
    ...
    # Determine variables
    val_label = ['urban_irri', 'pop_diff', 'MAP', 'MAT']
    col_label = ['MAP', 'MAP', 'urban_irri', 'uhi']
...
plt.tight_layout()
plt.savefig(os.path.join('..', '4_Figures', 'shap_scatter_modis.png'),
            dpi=900, bbox_inches='tight')
plt.show()