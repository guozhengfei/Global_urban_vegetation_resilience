...
if __name__ == '__main__':
    ...
    for sd_name in sd_names:
        ...
        for id in IDs[1:]:
            resis = np.load(current_dir + '/2_Output/Landsat_recovery_resistance/resistance_' + str(
                id) + '_smith_' + sd_name + 'sd.npy')
            resil = np.load(current_dir + '/2_Output/Landsat_recovery_resistance/resilience_' + str(
                id) + '_smith_' + sd_name + 'sd.npy')
            ...
            recovery_stats.append(
                [id, resis_inner, resis_sub, resis_rural, resil_inner, resil_sub, resil_rural, dt_vi_inner, dt_vi_sub,
                 dt_vi_rural])
        ...
        recovery_df.columns = ['ID', 'inner_rt', 'sub_rt', 'rural_rt', 'inner_recovery', 'sub_recovery',
                               'rural_recovery', 'dt_vi_inner', 'dt_vi_sub', 'dt_vi_rural']
        columns_to_check = ['inner_rt', 'sub_rt', 'rural_rt', 'inner_recovery', 'sub_recovery', 'rural_recovery',
                            'dt_vi_inner', 'dt_vi_sub', 'dt_vi_rural']
