...
for name in filenames:
    if name.startswith('urban_label') and name.endswith('.npy'):
        names.append(name)
IDs = []
for name in names:
    id = name.split('_')[-2]
...
for sd_name in sd_names:
    for id in IDs[1:]:
        resis = np.load(current_dir+'/2_Output/Modis_recovery_resistance/resistance_'+str(id)+ '_smith_'+sd_name+'sd.npy')
        resil = np.load(current_dir+'/2_Output/Modis_recovery_resistance/resilience_'+str(id)+ '_smith_'+sd_name+'sd.npy')
...
ax2.text(3, ax2.get_ylim()[1] * 0.95, 'c', ha='center', va='top', fontsize=10, fontweight='bold')
...
TACs_mean = np.load(current_dir+relative_path)['array1'] # v5,v4.2
...
df_merge = pd.merge(recovery_df,df_tac,on='ID')#.dropna()
