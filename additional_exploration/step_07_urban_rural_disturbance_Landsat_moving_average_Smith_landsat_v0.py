...
if __name__ == '__main__':
    ...
    for sd_name in sd_names:
        for id in IDs[1:]:
            ...
            vi_i = np.nanmedian(vis, axis=0)
            vi_i_rsp = np.reshape(vi_i, (int(vi_i.shape[0] / 12), 12))
            vi_i_sea = np.nanmedian(vi_i_rsp, axis=0)
            # plt.figure();plt.plot(vi_i_sea)
            vi_threshold = np.min([np.nanpercentile(vi_i_sea, 20), 0.2])

            gs_mask0 = vi_i_sea > vi_threshold
            gs_mask = np.repeat(gs_mask0[np.newaxis], 24, axis=0).reshape(-1)
            ...
            gsm_arr = gsm_arr + np.repeat(vis_mean, ser.shape[1], axis=0).T

            mean_all = np.nanmean(gsm_arr, axis=1)
            std = np.nanstd(gsm_arr, axis=1)

            p10 = np.nanpercentile(gsm_arr,10,axis=1)

            # plt.figure(); plt.plot(gsm_arr[855,:])

            # Identify extreme events for each pixel
            threshold = mean_all - 0.1*int(sd_name) * std  # threshold for extreme events
            ...
            print(f"Processed ID {sd_name, id}: {np.sum(np.any(extreme_events, axis=1))} pixels with extreme events")