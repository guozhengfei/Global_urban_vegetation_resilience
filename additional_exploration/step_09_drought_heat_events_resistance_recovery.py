...
if __name__ == '__main__':
    ...
    for id in IDs[1:]:
        ...
        res0[:,~gs_mask] = np.nan
        mask = (np.nanmean(vis, axis=1)<0.5) #
    ...
    # resis_inner = np.nanmean(np.nanmean(vis,axis=1)[urban_lab == 2], axis=0)
    # resis_sub = np.nanmean(np.nanmean(vis,axis=1)[urban_lab == 1], axis=0)
    # resis_rural = np.nanmean(np.nanmean(vis,axis=1)[urban_lab == 0], axis=0)
    #
    # vi_inner = res0[urban_lab == 2,:]
    # plt.figure(); plt.plot(np.nanmean(vi_inner,axis=0))
    #
    # vi_edge = res0[urban_lab == 1, :]
    # plt.plot(np.nanmean(vi_edge,axis=0))
    #
    # vi_rural = res0[urban_lab == 0, :]
