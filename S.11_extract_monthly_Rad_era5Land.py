...
for n in range(0, 23*12):
    ...
    evi = ((ee.ImageCollection('ECMWF/ERA5_LAND/MONTHLY_AGGR')
              ...
              .select('surface_solar_radiation_downwards_sum')).mosaic();
    imagecollections.append(evi)

FVC = ee.ImageCollection.fromImages(imagecollections).select('surface_solar_radiation_downwards_sum').toBands()
...
cities = dfc['ID']
folder = 'solarRad_ear5Land_urban'
...
for city in cities[2:]:
    ...
    dscr = 'Rad_monthly_'+str(geo['ID'][0])
    print(dscr)
