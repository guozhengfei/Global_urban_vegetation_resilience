...
def process_id(id):
    ...
    Emis = tf.imread(emis_folder + '/MOD21C3_emis_mean_2001_2023_' + str(id) + '.tif')
    if Emis.ndim == 3:
        Emis = np.nanmean(Emis, axis=2)
    Emis = cv2.resize(Emis, (lst.shape[1], lst.shape[0]), cv2.INTER_LINEAR)

    Rn = (1 - Alb) * SW + Emis * LW - Emis * 5.67037442 * 10 ** -8 * (lst + 273.15) ** 4
    ...
    top_100_SW_core = SW_flat[top_100_indices_core][top_100_vegC_core>0.95]
    top_100_LW_core = LW_flat[top_100_indices_core][top_100_vegC_core>0.95]
    top_100_Alb_core = Alb_flat[top_100_indices_core][top_100_vegC_core>0.95]
    top_100_emissivity_core = emissivity_flat[top_100_indices_core][top_100_vegC_core>0.95]
    ...
    top_100_SW_bg = SW_flat[top_100_indices_bg][top_100_vegC_bg>0.95]
    top_100_LW_bg = LW_flat[top_100_indices_bg][top_100_vegC_bg>0.95]
    top_100_Alb_bg = Alb_flat[top_100_indices_bg][top_100_vegC_bg>0.95]
    top_100_emissivity_bg = emissivity_flat[top_100_indices_bg][top_100_vegC_bg>0.95]
    ...
    SW_core = np.nanmean(top_100_SW_core)
    LW_core = np.nanmean(top_100_LW_core)
    albedo_core = np.nanmean(top_100_Alb_core)
    emissivity_core = np.nanmean(top_100_emissivity_core)
    ...
    SW_bg = np.nanmean(top_100_SW_bg)
    LW_bg = np.nanmean(top_100_LW_bg)
    albedo_bg = np.nanmean(top_100_Alb_bg)
    emissivity_bg = np.nanmean(top_100_emissivity_bg)
    ...
    return [id, Tc_core, Ta_core, ndvi_core, Rn_core, Q_core, H_core, SW_core, LW_core,
            albedo_core, emissivity_core,
            Tc_bg, Ta_bg, ndvi_bg, Rn_bg, Q_bg, H_bg, SW_bg, LW_bg,
            albedo_bg, emissivity_bg, uhi]

## main ##
project1_dir = '/Volumes/Zhengfei_02/Zhengfei_01-BACKUP/project 1 urban vegetation thermoregulation'
lst_dir_path = os.path.join(project1_dir, '1_Input', 'LST')

treeC_folder = os.path.join(project1_dir, '1_Input', 'treeCover_100m')

grassC_folder = os.path.join(project1_dir, '1_Input', 'grassCover_100m')
...
ndvi_folder = os.path.join(project1_dir, '1_Input', 'NDVI')
...
dem_folder = os.path.join(project1_dir, '1_Input', 'DEM_100m')

sw_folder = os.path.join('..','..','urban_env_data','shortwave')
lw_folder = os.path.join('..','..','urban_env_data','longwave')
alb_folder = os.path.join(project1_dir, '1_Input', 'Albedo')
emis_folder = '/Volumes/Zhengfei_01/urban_env_data/MOD21C3_emissivity_2001_2023_mean'
ratios = pd.read_csv(os.path.join('..','..','urban_env_data','noontime_energy_ratio.csv'))
ahe_folder = os.path.join(project1_dir, '1_Input', 'AHE_100m')
...
columns = [
    'ID', 'Tc_core', 'Ta_core', 'ndvi_core', 'Rn_core', 'Q_core', 'H_core',
    'SW_core', 'LW_core',
    'albedo_core', 'emissivity_core',
    'Tc_bg', 'Ta_bg', 'ndvi_bg', 'Rn_bg', 'Q_bg', 'H_bg',
    'SW_bg', 'LW_bg',
    'albedo_bg', 'emissivity_bg', 'uhi'
]
