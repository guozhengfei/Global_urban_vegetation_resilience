...
relative_path = '/2_Output/tac_nadir_city_3zones_STL.npz'
...
cities.plot('urban_core',ax=ax[0], marker='o',  markersize=5, cmap='RdBu_r',vmin=0.25,vmax=0.75)
ax[0].set_ylim([-54,70])
ax[0].set_xlim([-170,170])

cities.plot('urban_edge',ax=ax[1], marker='o',  markersize=5,cmap='RdBu_r',vmin=0.25,vmax=0.75)
ax[1].set_ylim([-54,70])
ax[1].set_xlim([-170,170])

# plt.figure(); plt.hist(TACs_mean.reshape(-1),50)

cities.plot('rural_bgr',ax=ax[2], marker='o',  markersize=5,cmap='RdBu_r',vmin=0.25,vmax=0.75)
ax[2].set_ylim([-54,70])
ax[2].set_xlim([-170,170])

ax[3].bar(np.linspace(0,2,3),tac_global_mean,yerr=np.nanstd(TACs_mean,axis=0)*0.25,width=0.45,color=['#7fc97f', '#beaed4', '#fdc086'])
ax[3].set_ylim([0.3,0.45])
ax[3].set_xticks([0,1,2],['urban_core','urban_edge','rural_bgr'])

figToPath = current_dir + '/4_Figures/Fig01a_tac_3zones_STL'
fig.tight_layout()
